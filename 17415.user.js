...
function removeLeftMenu() {
    if (elmOptionalBox && (getComputedStyle(elmLeft, '').display != 'none') && (elmOptionalBox.firstElementChild.nodeName != 'H4')) {
        elmLeft.parentNode.replaceChild(elmOptionalBox, elmLeft);
    }
    else {
        elmLeft.parentNode.removeChild(elmLeft);
    }
}
